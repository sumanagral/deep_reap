...
system that couples the two by injecting REAP's demand forecast
directly into the state representation of a convolutional DRL
scheduler. We contribute a complete, reproducible implementation:
a genetic-algorithm feature selector feeding a five-model
accuracy-weighted regression ensemble, a discrete-time cluster
environment, a CNN policy pre-trained by imitation of a
shortest-job-first expert and refined with Proximal Policy
Optimization, a REST integration layer, and Prometheus/Grafana
monitoring. On a 60-day synthetic trace, the REAP ensemble attains an
MSE of 8.07 on the CPU-load target, improving on every constituent
regressor. On a shared 3\,000-job scheduling trace, the DeepREAP
...
2.4$\times$ improvement --- while also lowering average completion
time from 49.51 to 19.04 time slots. We report these gains alongside
their costs: the same policy completes fewer jobs within a fixed step
budget, and our PPO refinement stage, run at a deliberately small
budget, degrades rather than improves the imitation checkpoint. All
code, seeds and figures are released to support replication.
...
\begin{enumerate}
\item \textbf{A working DeepREAP implementation.} We realize the full
architecture --- REAP prediction, DRL scheduling, an integration
protocol, and a monitoring loop --- as runnable software rather than a
specification.
\item \textbf{A concrete state-fusion mechanism.} We show how a
demand forecast can be injected into a DRL scheduler's state as
...
\item \textbf{An empirical evaluation against heuristic and DRL
baselines} on a shared workload trace, quantifying both where the
integration helps and where it does not.
\item \textbf{An honest account of the negative results.} Our PPO
refinement stage does not improve on the imitation checkpoint at the
training budget we could afford, and the ensemble does not dominate
its base models on every prediction target. We report both.
\end{enumerate}

\subsection{Research Questions}

\begin{description}
\item[RQ1] How can regression models be integrated into the
DeepRM\_Plus system to improve prediction of cloud resource demand?
\item[RQ2] What impact does that integration have on system
performance, job completion time and resource utilization?
\item[RQ3] To what extent can the integration reduce operational costs
for SMEs?
\end{description}
...
Son and Buyya~\cite{son2019} propose Priority-Aware VM Allocation
(PAVA) with Bandwidth Allocation (BWA) in SDN-enabled clouds,
improving response times and reducing QoS violations without
additional operational cost. Savitha and Salvi~\cite{savitha2021}
extend the idea with application-level priorities learned from
historical usage. Both are validated in simulation, and both depend on
the availability and consistency of historical data.

\subsection{Deep Reinforcement Learning for Scheduling}

Mao et al.~\cite{mao2016} introduced DeepRM, formulating cluster
scheduling as an RL problem over an image-like state encoding and
outperforming classical heuristics in simulation. Guo
et al.~\cite{guo2021} extended this into DeepRM\_Plus, combining deep
RL with imitation learning to accelerate convergence to a good policy
and improving average weighted turnaround and cycling time. Zhou
et al.~\cite{zhou2021} pair a trained DRL agent with a policy set that
enforces SLA and safety constraints. The open questions across this
line of work concern generalizability across platforms, scalability to
larger deployments, and the substantial data appetite of the training
procedures.

\subsection{Predictive Regression Models}

Bankole and Ajila~\cite{bankole2013} compare SVM, neural networks and
linear regression for provisioning prediction, finding SVM strongest
at 4.5\,\% average error and showing that including SLA metrics
improves accuracy further. Osypanka and
Nawrocki~\cite{osypanka2022,osypanka2023} develop ARIMA-based and
self-adapting long-term prediction systems, reporting cost reductions
of up to 85\,\% without QoS degradation. Gyeera
et al.~\cite{gyeera2023} apply Boosted Decision Trees to data-centre
KPI forecasting, achieving $R^2 = 0.9991$ on real-world data, though
at appreciable training cost.

\subsection{REAP}

Kaur, Bala and Chana~\cite{kaur2019} propose REAP, which applies a
genetic algorithm to select relevant features from historical data,
trains multiple regressors on the selected subset, and combines them
through an accuracy-weighted ensemble. Evaluated on a real-world
dataset, REAP reaches 95\,\% accuracy, outperforming individual
regression algorithms. Its costs are training-time expense and
uncertain transferability across cloud environments.

\subsection{The Gap}

The literature demonstrates that DRL schedules well and that
regression ensembles predict well. What remains underexplored is their
integration into a single framework, in which anticipated demand
conditions the scheduling policy itself. That integration --- and its
empirical evaluation --- is the subject of this paper.

% =====================================================================
\section{System Architecture}
% =====================================================================

DeepREAP comprises four modules: demand prediction, resource
scheduling, integration, and monitoring.
...
(resource\_usage.csv, job\_trace.csv)\\
$\downarrow$\\
\textbf{REAP:} GA feature selection $\rightarrow$ 5 regressors
$\rightarrow$ accuracy-weighted ensemble\\
$\downarrow$ \textrm{predicted CPU / memory demand}\\
\textbf{DeepRM\_Plus:} state image $\oplus$ REAP forecast;
imitation (SJF) $\rightarrow$ PPO\\
$\downarrow$ \textrm{scheduling decisions}\\
\textbf{FastAPI service:}
/predict /forecast /schedule /feedback /metrics\\
$\downarrow$ \textrm{/metrics scrape}\\
Prometheus $\rightarrow$ Grafana
\end{center}
\vspace{1mm}
\end{minipage}}
\caption{High-level architecture of the DeepREAP system. REAP
forecasts are fused into the scheduler's state representation rather
than post-processed, so the policy learns how much to trust them.}
\label{fig:arch}
\end{figure}

\subsection{Demand Prediction Module (REAP)}

\textbf{Input:} historical CPU, memory and network telemetry.
\textbf{Method:} GA-based feature selection followed by an
accuracy-weighted ensemble of five regressors.
\textbf{Output:} predicted per-resource demand for a given
(timestamp, service type) context.

\subsection{Resource Scheduling Module (DeepRM\_Plus)}

\textbf{State space:} an image encoding of cluster occupancy and
pending jobs, augmented with REAP forecast channels.
\textbf{Action space:} schedule one of $M$ visible jobs, or no-op.
\textbf{Reward:} negative sum of inverse job durations over all jobs
resident in the system, penalizing slowdown.
\textbf{Method:} a CNN policy trained by imitation learning and
refined with policy-gradient RL.

\subsection{Integration Layer}

The DeepREAP Integration Protocol (DRIP) carries REAP predictions to
the scheduler and returns allocation decisions for validation and
feedback. We implement it over a REST API with JSON payloads.

\subsection{Monitoring and Feedback Loop}

Prometheus scrapes service metrics; Grafana visualizes them. Observed
values submitted to a feedback endpoint update a prediction-error
gauge, closing the loop between realized and forecast demand.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{reap_quality_cpu_load.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{reap_quality_memory_usage.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{avg_slowdown.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{avg_completion.png}
...
\textbf{Synthetic workload.} Our trace is parametric. Although it
reproduces diurnal cycles, bimodal durations and dominant-resource
demand patterns, it is generated by a process we also control, and a
predictor may find it more forecastable than a production trace would
be. The magnitude of the DeepREAP gain in particular should be read as
an upper bound.

\textbf{Single trace, single seed.} Results come from one benchmark
trace at one seed. We report no confidence intervals and make no
significance claims.

\textbf{Short evaluation budget.} A 201-step budget truncates long
jobs and therefore interacts with the deferral behaviour that drives
DeepREAP's slowdown advantage. A longer horizon would let deferred
jobs complete and would likely narrow the gap in $n_{\text{done}}$.

\textbf{Simulation, not deployment.} The scheduler runs in a
simulated cluster. Network latency, failure handling and multi-tenancy
are absent.
...
Priorities for future work follow directly:

\begin{enumerate}
\item \textbf{Scale the RL budget.} A $10\times$ longer PPO run is the
single most valuable next experiment; the environment, network and GAE
pipeline already support it.
\item \textbf{Real traces.} Substituting the Google cluster trace or
Azure VM trace is a single-callsite change, as the generator already
emits the canonical schema.
\item \textbf{Sharper ensemble weighting.} Softmax-with-temperature or
top-$k$ weighting should address the memory-target result in
Table~\ref{tab:reap-mem}.
\item \textbf{Throughput-aware reward.} A reward combining slowdown
with completion count would let the policy trade the two explicitly
rather than implicitly.
\item \textbf{Online co-training.} REAP is currently fitted once and
frozen; re-fitting ensemble weights from live feedback would close the
loop the original architecture described.
\item \textbf{Cost modelling.} Pairing the simulator with a provider
billing model is the minimum needed to address RQ3.
\end{enumerate}

\subsubsection{Reproducibility.}
All code, fixed seeds, trained model checkpoints, raw benchmark
output and the scripts generating every figure in this paper are
